...
public static class AssetCatalog
{
    public static List<MapEntry> Parse(string json, string commit)
    {
        ...
        {
            if (!entry.IsAssetPack || entry.Id.Split('/').Length != 2 || string.IsNullOrWhiteSpace(entry.Name)
            ...
            if (entry.NotesPath != $"{entry.Id}/{entry.Version}/README.md") throw new InvalidDataException("Invalid asset release notes path.");
            ...
            foreach (var file in entry.Files)
            {
                ...
                if (source.Length != 4 || source[0] != "releases" || source[1] != "download" || !source[2].EndsWith("-" + entry.Version, StringComparison.Ordinal)
                    || !SafePaths.IsEditorAsset(file.Destination) || !paths.Add(file.Destination) || file.Size < 0 || file.Size > 2L * 1024 * 1024 * 1024)
